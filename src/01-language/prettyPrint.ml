...
(** The grade-dependent printers are parameterised by a grade system, since they
    pattern-match on the grade-dependent AST of [Ast.Make (GS)]. *)
module Make (GS : GradeSystem.S) = struct
  module Ast = Ast.Make (GS)
  module ResourceGrade = GS.ResourceGrade
  open Ast

  let print_resource_grade rho_pp =
    let rec aux (rho : resource_grade) ppf =
      match rho with
  ...
  let print_ty ?max_level ty_print_param rho_print_param =
    let rec aux ?max_level p ppf =
      ...
      | TyArrow (ty1, CompTy (ty2, rho)) ->
          print ~at_level:3 "%t → %t # %t" (aux ~max_level:2 ty1)
            (aux ~max_level:3 ty2)
            (print_resource_grade rho_print_param rho)
      | TyTuple [] -> print "unit"
      | TyTuple tys ->
          print ~at_level:2 "%t"
      ...
      | TyHandler (CompTy (ty1, rho1), CompTy (ty2, rho2)) ->
          print ~at_level:3 "%t # %t ⇒ %t # %t" (aux ~max_level:2 ty1)
            (print_resource_grade rho_print_param rho1)
            (aux ~max_level:3 ty2)
            (print_resource_grade rho_print_param rho2)
    in
    aux ?max_level
  ...
  and print_expression ?max_level e ppf =
    ...
    | Tuple lst ->
        Print.print_tuple
          (fun ?max_level e ppf -> print_expression ?max_level e ppf)
          lst ppf
    ...
    | Variant (lbl, Some arg) ->
        print ~at_level:1 "%t %t" (Label.print lbl)
          (print_expression ~max_level:0 arg)
    | Lambda (p, c) ->
        print ~at_level:2 "@[<hv 2>fun %t ↦@ %t@]" (print_pattern p)
          (print_computation ?max_level:None c)
    | PureLambda (p, c) ->
        print ~at_level:2 "@[<hv 2>fun %t ↦@ %t@]" (print_pattern p)
          (print_computation ?max_level:None c)
    ...
    | Handler (ret_case, op_cases) ->
        let print_op_cases ppf =
          List.iter
            (fun op_case -> Format.fprintf ppf "@,| %t" (print_op_case op_case))
            (OpNameMap.bindings op_cases)
        in
        print "@[<v 0>handler@,| return %t%t@]"
          (print_abstraction ret_case)
          print_op_cases
  ...
  and print_computation ?max_level c ppf =
    ...
    match c with
    | Return e ->
        print ~at_level:1 "return %t" (print_expression ~max_level:0 e)
    | Do (c1, (PNonbinding, c2)) ->
        print ~at_level:2 "@[<v 0>%t;@,%t@]"
          (print_computation ~max_level:1 c1)
    ...
    | Match (e, lst) ->
        let print_cases ppf =
          List.iter
            (fun case -> Format.fprintf ppf "@,| %t" (print_case case))
            lst
        in
    ...
    | Perform (op, e, (pat, c)) ->
        print ~at_level:1 "@[<hv 2>perform %t %t (%t.@ %t)@]" (OpName.print op)
          (print_expression ~max_level:0 e)
          (print_pattern pat)
          (print_computation ~max_level:1 c)
    | Handle (c, h) ->
        print ~at_level:1 "@[<v 0>handle@;<1 2>%t@,with %t@]"
          (print_computation c)
          (print_expression ~max_level:0 h)
  ...
  and print_op_case (op, a) ppf =
    Format.fprintf ppf "%t %t" (OpName.print op) (print_abstraction a)
  ...
  let print_vars_and_tys print_var_and_ty lst ppf =
    let rec print_list = function
      ...
      | ResourceGrade n :: rest ->
          let rho_pp = ResourceGradePrintParam.create () in
          print_resource_grade rho_pp n ppf;
          Print.print ppf "\n";
          print_list rest
    in
  ...
  let string_of_variable_context context =
    print_variable_context context Format.str_formatter;
